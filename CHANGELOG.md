...
- Type-ahead autocomplete is back in the editor, starting with dates: inside a `[[` link, type a natural-language date like `[[next monday` and pick the suggestion to drop in that day's note link (`[[2026-08-24]]`). Enter/Tab accepts, Escape dismisses. It's built on a small completion-source layer, so note, tag, and template suggestions can slot in next.
